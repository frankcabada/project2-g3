...
//			System.out.println(angleList);
	...
	private ArrayList<Integer> generateAngleListOfNearbyCells(Cell player_cell, Set<Cell> nearby_cells, boolean ignoreSamePlayer){
		// each element is in the form [left_angle, right_angle]
		ArrayList<Integer[]> cellAngles = new ArrayList<>(); 
		for(Cell c : nearby_cells){
			...
			double r = c.getDiameter() / 2.0;
			double theta1 = Math.atan((dY-r)/dX); 
			double theta2 = Math.abs(angle - theta1); // angle diff from center to edge of cell
			...
			double left_angle = (int)Math.toDegrees(angle + theta2);
			double right_angle = (int)Math.toDegrees(angle - theta2);
//			System.out.println(player_cell.hashCode() + "\t" + dY/dX);
//			System.out.println(Math.toDegrees(angle));
			cellAngles.add([left_angle, right_angle]);
		}
		Collections.sort(cellAngles);
		return cellAngles;
	}
